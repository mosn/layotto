...
type PostgresqlAlloc struct {
	Key        string     // Used to differentiate business biz_tag
	...
	Buffer     []*Segment // Two buffers, one are used as pre cache
...
// NewPostgresqlSegment For example, the maxid in the initialization DB is 1, that is, the number segment starts from 1,
//the step size is 1000, the maximum value is 1000, and the range is 1 ~ 1000. The next range is 1001 ~ 2000
