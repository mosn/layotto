...
func Test_GetSegment(t *testing.T) {
	...
	log.Info("get next id: %d", id)
	assert.NoError(t, err)
}

func Test_GetID_mock(t *testing.T) {
	//Because the dual buffer+ segment mode is adopted, I adopt a layered architecture.
	//If you use mock to mock things, you need to modify it at the Dao layer and mock it Expectbegin()

	//But that will destroy the original code structure, so if you want to verify whether there is a problem,
	//you can start a PostgreSQL with docker, and then execute the above test

	//Then execute postgresql SQL script,
	//and then configure the user password and other information in the configuration file to verify
