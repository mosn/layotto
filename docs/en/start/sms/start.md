...
## step 2. Run the client program to invoke Layotto SmsService API
<!-- tabs:start -->
### **Go**
Build and run the golang demo:

```shell
 cd ${project_path}/demo/sms/common/
 go build -o client
 ./client -s "demo"
```
...
```shell @if.not.exist examples-sms/target/examples-sms-jar-with-dependencies.jar
# build example jar
...
```shell
java -jar examples-sms/target/examples-sms-jar-with-dependencies.jar
```
...
The demo client program uses the SDK provided by Layotto to invoke the Layotto SmsService API.
