...
func TestGetFile(t *testing.T) {
	...
	a := NewAPI("", nil, nil, nil, nil, nil, map[string]file.File{"mock": mockFile}, nil, nil, nil, nil)
	var apiForTest = a.(*api)
	err := apiForTest.GetFile(&runtimev1pb.GetFileRequest{StoreName: "mock1"}, mockStream)
	...
	go SendData(w)
	err = apiForTest.GetFile(&runtimev1pb.GetFileRequest{StoreName: "mock"}, mockStream)
	assert.Nil(t, err)
}
...
func TestListFile(t *testing.T) {
	...
	resp, err := apiForTest.ListFile(context.Background(), &runtimev1pb.ListFileRequest{Request: request})
	assert.Nil(t, resp)
	assert.Equal(t, err, status.Errorf(codes.InvalidArgument, "not support store type: mock1"))
	...
	_, err = apiForTest.ListFile(context.Background(), &runtimev1pb.ListFileRequest{Request: request})
	assert.NotNil(t, err)
...
func TestDelFile(t *testing.T) {
	...
	request.Metadata = make(map[string]string)
	var apiForTest = a.(*api)
	resp, err := apiForTest.DelFile(context.Background(), &runtimev1pb.DelFileRequest{Request: request})
	assert.Nil(t, resp)
	assert.Equal(t, err, status.Errorf(codes.InvalidArgument, "not support store type: mock1"))
	request = &runtimev1pb.FileRequest{StoreName: "mock", Name: "test"}
	request.Metadata = make(map[string]string)
	...
	_, err = apiForTest.DelFile(context.Background(), &runtimev1pb.DelFileRequest{Request: request})
	assert.Nil(t, err)
...
func TestGetFileMeta(t *testing.T) {
	...
	resp, err := apiForTest.GetFileMeta(context.Background(), request)
	assert.Nil(t, resp)
